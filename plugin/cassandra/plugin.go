...
func (p CassandraPlugin) Validate(log io.Writer, endpoint plugin.ShieldEndpoint) error {
	var (
		s    string
		err  error
		fail bool
	...
	if err != nil {
		...
		fmt.Fprintf(log, "@G{\u2713 host}          using default node @C{%s}\n", DefaultHost)
	} else {
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 port          %s}\n", err)
		fail = true
	} else if s == "" {
		fmt.Fprintf(log, "@G{\u2713 port}          using default port @C{%s}\n", DefaultPort)
	} else {
		fmt.Fprintf(log, "@G{\u2713 port}          @C{%s}\n", s)
	}

	s, err = endpoint.StringValueDefault("user", "")
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 user          %s}\n", err)
		fail = true
	} else if s == "" {
		fmt.Fprintf(log, "@G{\u2713 user}          using default user @C{%s}\n", DefaultUser)
	} else {
		fmt.Fprintf(log, "@G{\u2713 user}          @C{%s}\n", s)
	}
	...
	if err != nil {
		...
		fmt.Fprintf(log, "@G{\u2713 password}      using default password @C{%s}\n", DefaultPassword)
	} else {
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 keyspace      %s}\n", err)
		fail = true
	} else {
		fmt.Fprintf(log, "@G{\u2713 keyspace}      @C{%s}\n", s)
	}

	s, err = endpoint.StringValueDefault("datadir", "")
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 datadir       %s}\n", err)
		fail = true
	} else if s == "" {
		fmt.Fprintf(log, "@G{\u2713 datadir}       using default @C{%s}\n", DefaultDataDir)
	} else {
		fmt.Fprintf(log, "@G{\u2713 datadir}       @C{%s}\n", s)
	}
...
func (p CassandraPlugin) Backup(out io.Writer, log io.Writer, endpoint plugin.ShieldEndpoint) error {
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Clean any stale snapshot}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Clean any stale snapshot}\n")
	...
	defer func() {
		...
		err = plugin.Exec(cmd, nil, out, log)
		if err != nil {
			fmt.Fprintf(log, "@R{\u2717 Clean snapshot}\n")
			return
		}
		fmt.Fprintf(log, "@G{\u2713 Clean snapshot}\n")
	}()
	...
	err = plugin.Exec(cmd, nil, out, log)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Create new snapshot}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Create new snapshot}\n")
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Create base temporary directory}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Create base temporary directory}\n")
	...
	err = plugin.Exec(cmd, nil, out, log)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Clear base temporary directory}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Clear base temporary directory}\n")
	...
	defer func() {
		...
		err = plugin.Exec(cmd, nil, out, log)
		if err != nil {
			fmt.Fprintf(log, "@R{\u2717 Clean base temporary directory}\n")
			return
		}
		fmt.Fprintf(log, "@G{\u2713 Clean base temporary directory}\n")
	}()

	plugin.DEBUG("Creating directory '%s' with 0700 permissions", tmpKeyspaceDir)
	err = os.Mkdir(tmpKeyspaceDir, 0700)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Create temporary directory}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Create temporary directory}\n")
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Recursive hard-link snapshot files}\n")
		return err
	}
	if !info.IsDir() {
		fmt.Fprintf(log, "@R{\u2717 Recursive hard-link snapshot files}\n")
	...
	for _, tableDirInfo := range entries {
		...
		if err != nil {
			fmt.Fprintf(log, "@R{\u2717 Recursive hard-link snapshot files in temp dir}\n")
			return err
		}
	}
	fmt.Fprintf(log, "@G{\u2713 Recursive hard-link snapshot files in temp dir}\n")
	...
	err = plugin.Exec(cmd, nil, out, log)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Set ownership of snapshot hard-links}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Set ownership of snapshot hard-links}\n")
	...
	err = plugin.Exec(cmd, nil, out, log)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Stream tar of snapshots files}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Stream tar of snapshots files}\n")
...
func (p CassandraPlugin) Restore(in io.Reader, log io.Writer, endpoint plugin.ShieldEndpoint) error {
	cassandra, err := cassandraInfo(endpoint)
	if err != nil {
		return err
	}
	...
	plugin.DEBUG("Executing `%s`", cmd)
	err = plugin.Exec(cmd, nil, log, log)
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Clear base temporary directory}\n")
		return err
	}
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Extract tar to temporary directory}\n")
		return err
	}
	fmt.Fprintf(log, "@G{\u2713 Extract tar to temporary directory}\n")
	...
	if err != nil {
		fmt.Fprintf(log, "@R{\u2717 Load all tables data}\n")
		return err
	}
	defer dir.Close()
