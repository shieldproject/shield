...
func SetupLogging(cfg LogConfig) {
	...
	if cfg.Type == "syslog" {
		...
		if err != nil {
			l.out = os.Stdout
			os.Stderr.Write([]byte(fmt.Sprintf("Unable to hook up to syslog, using console for logging: %s", err)))
		}
		l.out = logger
		...
		if err != nil {
			l.out = os.Stdout
			os.Stderr.Write([]byte(fmt.Sprintf("Unable to log to %s - using console instead: %s", cfg.File, err)))
		}
		l.out = f
	} else {
		if cfg.File == "stderr" {
